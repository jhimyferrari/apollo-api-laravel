...
describe('POST api/sellers/{seller}/address', function () {
    test('Logged user with valid data', function () {
        ...
        $response = $this->postJson(route('v1.sellers.addresses.store', $seller), $data);
    ...
    test('Logged user with invalid data', function () {
        ...
        $response = $this->postJson(route('v1.sellers.addresses.store', $seller), []);
    ...
    test('Logged user with other organization seller', function () {
        $user = User::factory()->create();
        Sanctum::actingAs($user, [PermissionType::SELLER_UPDATE->value]);
    ...
    test('Logged user with valid data for non existent seller', function () {
        Sanctum::actingAs(User::factory()->create(), [PermissionType::SELLER_UPDATE->value]);
        $response = $this->postJson(route('v1.sellers.addresses.store', ['seller' => 'non-existent-id']), []);
