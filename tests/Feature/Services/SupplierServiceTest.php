...
describe('SupplierService', function () {
    ...
    describe('address', function () {
        ...
        describe('createAddress', function () {
            it('should create and add an address to a supplier', function () {
                $supplier = Supplier::factory()->create(['organization_id' => $this->user->organization_id]);
                ...
                $address = $this->service->createAddress($supplier, $data);
                ...
                expect($address)
                    ...
                    ->addressable->id->toBe($supplier->id);
            });

            it('should create add 2 address and change default', function () {
                $supplier = Supplier::factory()->create(['organization_id' => $this->user->organization_id]);
                $city_ibge_code = City::first()->ibge_code;
                $oldAddress = $supplier->addresses()->create(Address::factory()->turnDefault()->make(['organization_id' => $this->user->organization_id])->toArray());
                $data = [
                    'street' => 'Rua A',
                    'number' => '213',
                    'neighborhood' => 'centro',
                ...
                expect(
                    Address::query()
                        ->where('addressable_id', $supplier->id)
                        ->where('is_default', true)
                        ->count()
                )->toBe(1);

                expect($supplier->defaultAddress)->id->toBe($address->id);
        ...
        describe('setDefaultAddress', function () {
            ...
            it('should turn an Address to default', function () {
                $supplier = Supplier::factory()->create(['organization_id' => $this->user->organization_id]);
                $address = $supplier->addresses()->create(Address::factory()->for($this->user->organization)->make()->toArray());
                $this->service->setDefaultAddress($supplier, $address);
                $address->refresh();
            ...
            it('should turn false a default address after turn default other  ', function () {
                $supplier = Supplier::factory()->create(['organization_id' => $this->user->organization_id]);
                $addressOld = $supplier->addresses()->create(Address::factory()->turnDefault()->make(['organization_id' => $this->user->organization_id])->toArray());
                $address = $supplier->addresses()->create(Address::factory()->for($this->user->organization)->make()->toArray());
